...
async def produce_messages(loop, n, key, partition, csv1):
    producer = fake_kafka.AIOKafkaProducer(loop=loop, bootstrap_servers=['http://127.0.0.1:8000'])
...
async def consume_messages(loop, n, key, partition, csv2):
    consumer = fake_kafka.AIOKafkaConsumer("my_topic", loop=loop, bootstrap_servers=['http://127.0.0.1:8000'], group_id='a')
